...
description: "資格情報を保護するローカルプロキシを介してホステッド エージェントの Backer Concierge を統合し、ウィジェットをテストして Agent merge を使用します。"
authors:
  - juliamuiruri4
...
- レビューを終えて Agent merge に引き継いだ変更と、リソースのクリーンアップのチェックポイント。
...
2. Microsoft Foundry Canvas を再度開き、同じプロジェクト、デプロイ、ホステッド エージェントのステータスを確認します。リソースをクリーンアップした場合は、統合する前に、該当する[プロジェクトとモデル][project-module]および[テスト済みのホステッド エージェントのデプロイ][previous-module]を復元します。それ以外の場合は、別のプロジェクトを作成せずに再利用します。
...
ローカル統合がチェックに合格したら、Agent merge で同じ Issue にリンクされた変更を仕上げます。

12. 同じ Tailspin Toys worktree セッションで、エージェント、ルートの `azure.yaml`、ローカルプロキシ、ウィジェット、テストなど、変更したすべてのファイルをレビューします。匿名でアクセスできる公開プロキシのインフラストラクチャが追加されていないことを確認します。
13. 生成された環境ファイル、ローカル設定、トークン、資格情報が含まれていないことを確認します。
14. **Create PR** の横にあるドロップダウンを選択し、**Agent merge** を選択します。
15. **Agent merge** を選択して pull request を作成し、チェックを監視します。報告された失敗を確認して対処するまでは、変更を完了と見なさないでください。

## チェックポイントと次のステップ

最終チェックポイントには、ホステッド エージェントの検証の証拠と、ローカルで検証済みの Web サイト統合の両方が含まれます。プロキシやサイトを本番環境にデプロイしたものではありません。

16. 同じ Issue セッションに、プロキシとエンドツーエンドテストの結果、および pull request とチェックのステータスを記録します。記録には、3 つのモジュールで一貫して使用した同じ Tailspin Toys リポジトリ、worktree ブランチ、Foundry プロジェクト、モデルデプロイ、ホステッド エージェントのバージョンを明記してください。
17. 実験を終えたら、[リソースをクリーンアップする][cleanup]の手順に従い、両方のローカルサービスを停止し、ワークショップ専用の Azure リソースだけを削除します。
18. 既存のコアワークショップのルートにある[振り返りと次のステップ][core-review]に進みます。
