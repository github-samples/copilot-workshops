...
2. Send a small, well-scoped prompt. For example, you could delegate the stretch-goal pagination from your backlog — it's the issue labeled `workshop:pagination`:

    ```
    Implement the pagination issue from the backlog (labeled workshop:pagination) on the roles list page so it shows a fixed number of roles per page with Previous and Next controls, and add tests.
    ```
