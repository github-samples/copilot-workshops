...
```text
Using the Playwright MCP server, open a browser to the running app at http://localhost:4321 and verify the new role filtering feature:

1. Go to the roles page and note how many roles are listed.
2. Apply a department filter and confirm the list updates to only show roles in that department.
3. Clear the filter and confirm all roles are shown again.
4. Apply a different department filter and confirm the results update accordingly.
