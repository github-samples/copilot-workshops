---
title: "에이전트를 사이트에 연결"
description: "자격 증명을 안전하게 보호하는 로컬 프록시로 호스팅 Backer Concierge를 통합하고 위젯을 테스트한 다음 Agent merge를 사용합니다."
authors:
  - juliamuiruri4
...
- 검토를 마치고 Agent merge에 전달한 변경 내용과 리소스 정리 체크포인트
...
2. Microsoft Foundry Canvas를 다시 열고 동일한 프로젝트, 배포, 호스팅 에이전트 상태를 확인합니다. 리소스를 정리했다면 통합하기 전에 관련 [프로젝트와 모델][project-module] 및 [테스트한 호스팅 배포][previous-module]를 복원합니다. 그렇지 않으면 다른 프로젝트를 만들지 말고 기존 리소스를 재사용합니다.
...
로컬 통합이 검사를 통과하면 Agent merge로 동일한 이슈에 연결된 변경을 마무리합니다.

12. 동일한 Tailspin Toys 워크트리 세션에서 에이전트, 루트 `azure.yaml`, 로컬 프록시, 위젯, 테스트를 포함하여 변경한 모든 파일을 검토합니다. 익명 공개 프록시 인프라를 추가하지 않았는지 확인합니다.
13. 생성된 환경 파일, 로컬 설정, 토큰, 자격 증명이 포함되어 있지 않은지 확인합니다.
14. **Create PR** 옆의 드롭다운을 선택한 다음 **Agent merge**를 선택합니다.
15. **Agent merge**를 선택하여 끌어오기 요청을 만들고 검사 상태를 모니터링합니다. 변경을 완료한 것으로 간주하기 전에 보고된 실패를 검토하고 해결합니다.

## 체크포인트 및 다음 단계

최종 체크포인트는 호스팅 에이전트 검증 증거와 로컬에서 검증한 웹사이트 통합을 함께 포함합니다. 프록시나 사이트를 프로덕션에 배포한 것은 아닙니다.

16. 동일한 이슈 세션에 프록시 및 엔드투엔드 테스트 결과와 끌어오기 요청 및 검사 상태를 기록합니다. 세 모듈 전체에서 사용한 동일한 Tailspin Toys 리포지토리, 워크트리 브랜치, Foundry 프로젝트, 모델 배포, 호스팅 에이전트 버전을 기록에서 식별할 수 있는지 확인합니다.
17. 실험을 마치면 [리소스 정리][cleanup]를 따릅니다. 두 로컬 서비스를 모두 중지하고 워크숍 전용 Azure 리소스만 제거합니다.
18. 기존 핵심 워크숍 경로의 [검토 및 다음 단계][core-review]로 계속 진행합니다.
