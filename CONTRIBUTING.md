...
Thanks for your interest in contributing to *Agents in SDLC*! This repository hosts the workshop content (MDX source, the Astro + Starlight site that publishes it, and the renderer that produces the github.com-readable mirror).
...
- **`render-markdown.yml`** — renders MDX and validates the output (catches missing partials, unresolved component tags, broken links, missing images).
- **`pages.yml`** — builds the Astro site (must produce 29 pages).
- **Lychee** — link checks the rendered Markdown.

**Do not commit changes to `workshop/`.** That folder is regenerated by a bot on every push to `main`. Manual edits will be overwritten. (One-time exceptions: structural migrations that change the renderer's output may include a regenerated `workshop/` diff in the same PR — call this out in the PR description if it applies.)
