...
> [!NOTE]
> Do **not** edit anything under `workshop/` — it is regenerated from `workshop-content/` by `scripts/render-markdown.py`. CI will reject PRs that change `workshop/` without a corresponding source change.
...
**Render workshop Markdown (required when editing `workshop-content/`):**

```bash
python scripts/render-markdown.py
```

Commit the resulting `workshop/` changes in the same PR. The `render-markdown.yml` CI workflow will fail otherwise.
