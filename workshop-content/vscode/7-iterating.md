...
- 💻 **[CLI path](../cli/README.md)** — work the same flows from your terminal with Copilot CLI: plan mode, agent skills, custom agents, slash commands like `/share`, `/context`, and `/delegate` to bridge back to coding agent.
- ☁️ **[Cloud / Coding Agent path](../cloud/README.md)** — focus exclusively on assigning issues to coding agent, monitoring sessions through the agents page, and iterating asynchronously on pull requests.
