# Exercise 5: Iterating on GitHub Copilot's work

| [← Previous lesson: Managing agents][previous-lesson] |
|:--|

--8<-- "sections/iterating-recap.md"

--8<-- "sections/security-coding-agent.md"
...
- **Inspected the custom instruction files this repo ships with** so you could spot their effect in coding agent's output, connecting what you learned in Exercise 1 with the read-only inspection in Exercise 2.
- **Used Copilot coding agent to address tech debt and add features asynchronously.** You assigned the documentation issue and the CRUD endpoints issue, watching coding agent set up its environment, plan, and execute in the background.
...
- **Reviewed and iterated on coding agent's pull requests**, tagging `@copilot` to request changes and approving workflows.
...
You've completed the Cloud / Coding Agent path. If you'd like to keep exploring, the other paths complement what you practiced here:

- 🖥️ **[VS Code path](../vscode/README.md)** — explore Copilot Chat agent mode and MCP integration directly from your IDE.
- 💻 **[CLI path](../cli/README.md)** — work the same flows from your terminal with Copilot CLI: plan mode, agent skills, custom agents, and slash commands like `/delegate` to bridge back to the coding agent you used here.

In your own repository, try these follow-up ideas:

- Assign a refactoring or test-coverage issue to coding agent.
...
[previous-lesson]: ./4-managing-agents.md
