<!--
  ...
  Source: workshop-content/cloud/1-custom-instructions.mdx
...
1. From your codespace terminal (or a local clone), create and switch to a new branch:
...
## Commit and push your instruction update

Cloud agent reads instruction files from the branch the issue targets. Push this branch so it's available when you assign work to cloud agent in the next exercise.
...
2. Push the branch (and, optionally, open a PR / merge to `main` so cloud agent picks it up automatically when assigned issues on `main`):
