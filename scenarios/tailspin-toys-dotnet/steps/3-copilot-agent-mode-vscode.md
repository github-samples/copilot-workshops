...
The website is a crowdfunding platform for board games with a developer theme. It allows users to list games and display details about them. The website has two main components: the front-end (written in Svelte) and the backend (written in C#).
...
3. Run the following script to start the application:
   ...
   ```bash
   Server (ASP.NET Core) running at: http://localhost:5100
