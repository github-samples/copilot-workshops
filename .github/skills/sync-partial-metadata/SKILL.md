...
description: Regenerate the partial metadata artifacts (lint output and `docs/src/types/_partials/<name>.mdx.d.ts` declaration files that power VS Code hover tooltips) for the docs partials system. Use this skill whenever a partial under `docs/src/content/docs/_partials/` is created, edited, renamed, or removed; whenever a consumer page changes which partials it imports; or whenever an author asks an agent to "generate the partial types", "sync the partial metadata", "update the .d.ts files", or otherwise refresh the generated declaration files.
---
...
- A file under `docs/src/content/docs/_partials/*.mdx` is created, edited (including changes to the `@summary`/`@sections` metadata block, headings, or component name), renamed, or deleted.
- A consumer `.mdx` page (anywhere under `docs/src/content/docs/`) adds, removes, or renames a `import … from '@partials/…'` line.
...
1. **Lint first**. Run `python scripts/lint_partials.py` from the repo root. It validates:
    ...
    - imports use the `@partials/` alias (not relative `../_partials/...`),
...
2. **Sync**. Run `python scripts/sync_partial_metadata.py` from the repo root. This regenerates:
    - `docs/src/types/_partials/<name>.mdx.d.ts` for every partial (these provide JSDoc that powers VS Code hover tooltips at every `<Component />` use site),
    - migrates any stale `'../_partials/…'` imports in consumer pages to `'@partials/…'`.
...
4. **Stage the regenerated files**. Include any modified files under `docs/src/types/_partials/` in the same commit as the partial change. Never hand-edit those files — they carry a `// GENERATED FILE — do not edit` header and are marked read-only in `.vscode/settings.json`.
5. **(Optional) Render check**. If the change touched partial bodies or headings, also run `python scripts/render-markdown.py` to confirm `workshop/` regeneration still succeeds.
